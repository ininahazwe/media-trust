...
from routers import outlets, respondents, dashboard  # À adapter
...
app.add_middleware(
    ...
    allow_origins=[
        ...
        "https://votre-frontend.vercel.app",  # Production Vercel
    ],
    allow_credentials=True,
    allow_methods=["*"],
    allow_headers=["*"],
    allow_origin_regex=".*vercel\.app$",  # Accept all Vercel subdomains
)

# ✅ Health check endpoint (Render l'utilise)
@app.get("/health")
...
if __name__ == "__main__":
    ...
    uvicorn.run(
        "main:app",
        host="0.0.0.0",  # Render requiert ceci
        port=int(os.getenv("PORT", 8000)),  # Render injecte PORT
        reload=False  # Important en production
    )